...
Снимок состояния сервера снят вручную **2026-07-14** (Ubuntu 24.04.4 LTS, Hostinger, Литва,
...
- **Файрвол провайдера (Hostinger, hPanel)**: firewall самого хоста открыт полностью
  (`iptables INPUT` — `policy ACCEPT`, никаких ограничений). Если в панели Hostinger включён
  файрвол VPS — именно он фильтрует трафик снаружи. Список портов, которые должны быть
  разрешены на `0.0.0.0/0`, см. раздел «Порты» ниже. Из сервера это не проверить — сверять
  руками в hPanel.
...
## 4. Порты, которые должны быть открыты в файрволе провайдера (0.0.0.0/0)
