...
> ⛔ **СТРОГО ЗАПРЕЩЕНО делать `git commit`, `git push` или любые git-операции записи без явной команды пользователя.**  
> Нарушение этого правила недопустимо даже если кажется что "так удобнее".

> 📺 **Вывод команд через `.tmp.log`:** если команда перенаправляется в `.tmp.log`, сразу же после чтения файла показывать его содержимое пользователю через инструмент отображения.

> 🗂️ **Временные скрипты — только в `.tmp/`:** если нужно сгенерировать временный скрипт для выполнения — создавать его в папке `.tmp/`, запускать, затем удалять. Папка в `.gitignore`, в репозиторий не попадает.
...
> 🔄 **Идемпотентность скриптов:** Все временные и постоянные скрипты должны быть **максимально идемпотентными**.

**Требования:**
1. **Работают без параметров** — вывести usage/help:
   ```bash
   ./script.sh        # показать справку
   ./script.sh --help # показать справку
   ```

2. **Безопасны для множественного запуска** — можно запускать 100 раз подряд, не сломается ничего:
   - Проверять наличие файлов/сервисов перед созданием
   - Использовать `--force` флаг для переустановки, если нужно перезаписать
   - Не удалять данные без явного согласия
   - Использовать `set -euo pipefail` для безопасности
...
if [[ $# -eq 0 ]] || [[ "$1" == "--help" ]]; then
  echo "Usage: ./script.sh [--force]"
  echo "  --force    Переустановить даже если уже установлено"
  exit 0
fi

# Проверка: может ли быть запущено повторно
if [[ -f /etc/installed.marker ]] && [[ "${1:-}" != "--force" ]]; then
  echo "[✓] Уже установлено. Используйте --force для переустановки"
  exit 0
fi
...
---
**Правила структуры директорий см. в разделе "Структура репозитория" файла [README.md](README.md).**
...
---
**Детали о настройке PuTTY Session и пути к репозиторию на сервере см. в разделе "SSH-подключение" и "Начальная настройка нового хоста" файла [README.md](README.md).**
